...
W1_en = tf.Variable(tf.truncated_normal([784, 300], stddev = 0.1, seed = 123)) # encoder step
b1_en = tf.Variable(tf.zeros([300]))
W1_de = tf.transpose(W1_en) # decoder step
b1_de = tf.Variable(tf.zeros([784]))

W2_en = tf.Variable(tf.truncated_normal([300, 100], stddev = 0.1, seed = 123)) # encoder step
b2_en = tf.Variable(tf.zeros([100]))
W2_de = tf.transpose(W2_en) # decoder step
b2_de = tf.Variable(tf.zeros([300]))

W3 = tf.Variable(tf.truncated_normal([100, 10], stddev=0.1, seed = 123))
...
train_step1 = tf.train.GradientDescentOptimizer(0.05).minimize(loss1, var_list=[W1_en, b1_en, b1_de])
train_step2 = tf.train.GradientDescentOptimizer(0.05).minimize(loss2, var_list=[W2_en, b2_en, b2_de])
