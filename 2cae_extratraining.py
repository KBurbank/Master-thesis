...
x_err = tf.placeholder(tf.float32, shape = [None, 784])
y_err = tf.placeholder(tf.float32, shape = [None, 10])
zero = tf.constant(0, dtype = tf.float32)
index = tf.reduce_sum(tf.cast(tf.not_equal(y, y_), dtype=tf.float32), 1)
index_prime = tf.where(tf.not_equal(index, zero))
x_err = tf.squeeze(tf.gather(x, index_prime))
y_err = tf.squeeze(tf.gather(y_, index_prime))

x_err_data = sess.run(x_err, feed_dict= {x: mnist.train.images[1:1000, :],
                                         y_: mnist.train.labels[1:1000, :]})
y_err_data = sess.run(y_err, feed_dict = {x: mnist.train.images[1:1000, :],
                                          y_: mnist.train.labels[1:1000, :]})

x_err_data = np.asarray(x_err_data, dtype=float)
y_err_data = np.asarray(y_err_data, dtype = float)

for _ in range(1000):
    sess.run(train_step1, feed_dict={x:x_err_data})

for _ in range(1000):
    sess.run(train_step2, feed_dict={x:x_err_data})

for _ in range(1000):
    sess.run(train_step3, feed_dict={x: x_err_data, y_: y_err_data})
